...
from metabci.brainda.algorithms.decomposition import generate_cca_references, generate_filterbank, FBTDCA, FBSCCA, \
    FBTRCA,FBDSP
from metabci.brainda.algorithms.dynamic_stopping import bayes
from metabci.brainda.algorithms.utils.model_selection import set_random_seeds, match_loo_indices, generate_loo_indices
from metabci.brainda.datasets.tsinghua import Wang2016
import numpy as np
from metabci.brainda.paradigms import SSVEP
import mne
from mypy.nodes import YieldFromExpr
from psychopy.app.urls import general
...
dataset=Wang2016()
# 实例化数据集类
# 获取原始数据（结构：{subject: {session: {run: Raw}}}）
...
paradigm=SSVEP(
    srate=srate,
    channels=channels,
    intervals=[(start_pnt+delay,start_pnt+delay+duration+0.1)],
    events=events
)
