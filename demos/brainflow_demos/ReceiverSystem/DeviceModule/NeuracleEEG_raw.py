...
class NeuracleEEGThread(threading.Thread, ):
    def __init__(self, threadName, device, n_chan, hostname, port, srate, t_buffer):
        ...
        self.datalist = np.zeros((n_chan, 1), dtype=int)
        self.trial_num = 0
    ...
    def save_data(self, data):
        ...
        if block_start_trigger in data:
            self.datalist = np.zeros((self.n_chan, 1), dtype=int)
        if trial_end_trigger in data:
            cut_data = copy.deepcopy(self.datalist)
            index = data.index(trial_end_trigger)
            self.trial_num += 1
            print(index)
            with open('{}.pkl'.format(self.trial_num), 'wb') as f:
                # print(type(cut_data))
                save_data = cut_data[:, (index-4400):index]
                print(save_data.shape, 1)
        ...
        while self.shutdown_flag.isSet():
            ...
            for r in rs:
                ...
                try:
                    ...
                    # print(data)
                    self.datalist = np.concatenate([self.datalist, data], axis=1)
                    # print(self.datalist.shape)
                    self.save_data(self.datalist)
    ...
    def parseData(self, raw):
        if 'Neuracle' in self.device:  # parsa data according to Neuracle device protocol
            ...
            parse_data = []
        parse_data = np.asarray(parse_data)
        return parse_data
